...
function boolImage($value, $trueText = '', $falseText = '')
{
    ...
    if ($value) {
        return '<img src="/images/icon/tick.png" alt="' . html_escape($trueText) . '" title="' . html_escape(
            $trueText
        ) . '" />';
    } else {
        return '<img src="/images/icon/cross.png" alt="' . html_escape($falseText) . '" title="' . html_escape(
            $falseText
        ) . '" />';
    }
}
